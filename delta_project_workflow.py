...
#Set the high level directories and the number of workers 
...
ROI_shapefiles=['/Users/asparra/Documents/Global_deltas/data/Shapefiles/DeltaSHPs/global_map_MagdalenaBuff5km.shp']
...
if __name__ == '__main__':
    for i in ROI_shapefiles:
        ...
        for n in file_types:
            ...
            output_file=output_directory2+'/'+delta_name+'_complete_' + data_name +'.shp'
            #Check if the complete shapefile for the delta and for each data type already exists. If it does, skip to the next element.
            if os.path.isfile(output_file) or os.path.isfile(re.sub('.shp', '_0.shp', output_file)):
                continue
            else:
                ...
                if os.path.isfile(query_file):
                   ...
                   files = files[0].split(',')
                   available_files = [re.sub('[[]|[]]', '', file) for file in files] 
                   available_files = [file.replace("'", "")for file in available_files]
                   if re.search("No available files for this ROI", available_files):
                       continue
                else:   
                    if query_type: 
                ...
                if len(available_files)==0:
                    ...
                    dpf.parallel_workflow_per_file(available_files, temporary_directory, i, output_directory2, data_group, NUM_WORKERS)
                    ...
                    final_files = [item for item in available_files if re.split(".h5|.H5", item.split("/")[-1])[0]+'.shp' in os.listdir(output_directory2)]
                    ...
                    paths = [os.path.join(output_directory2, x) for x in files if ".shp" in x]
                    if 'ATLAS03' in output_file:
                        continue
                    else:    
                        if len(paths)!=0:
                            if len(paths)<225: #If there are more than 225 files create separate shapefile files per groups of 200 files
                                ...
                                for number in range(len(nums1)):
                                    output_file=re.sub('.shp', '_'+str(number)+'.shp', output_file) 
